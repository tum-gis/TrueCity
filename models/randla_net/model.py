...
# Inlined minimal pt_utils with Conv1d/Conv2d wrappers to avoid external dependency
class Conv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, bn=False, activation='relu', bias=None):
        super().__init__()
        if bias is None:
            bias = not bn
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, bias=bias)
        self.bn = nn.BatchNorm1d(out_channels) if bn else None
        self.act = None if activation is None else (nn.ReLU(inplace=True) if activation == 'relu' else nn.LeakyReLU(0.2, inplace=True))
    def forward(self, x):
        x = self.conv(x)
...
class Conv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=(1,1), stride=(1,1), padding=(0,0), bn=False, activation='relu', bias=None):
        super().__init__()
        if bias is None:
            bias = not bn
