...
class SemanticSegmentationModule(LightningModule):
    ...
    def __init__(
        ...
        self.test_cm_excl = ConfusionMatrix(num_classes)
    ...
    def test_step(self, batch: NAG, batch_idx: int) -> None:
        ...
        # ====== 新增：为回调提供 pos / pred（与提交分支同样的取法）======
        # 无论是否 submit，都给回调可用的数据
        nag = batch if isinstance(batch, NAG) else batch[0]
        l0_pos = nag[0].pos.detach().cpu()
        l0_pred = output.semantic_pred()[nag[0].super_index].detach().cpu()
        ...
        # 显式释放
        ret = {"pos": l0_pos, "pred": l0_pred}  # <== 新增返回值
        del output
        return ret
    ...
    def test_step_update_metrics(
        ...
        # ===== 原有全类指标 =====
        self.test_loss(loss.detach())
        pred_all = output.semantic_pred().detach()
        tgt_all = output.semantic_target.detach()
        
        # 类型保险（确保是long类型）
        pred_all = pred_all.long()
        tgt_all = tgt_all.long()
        
        # 更新原有的全类别混淆矩阵
        self.test_cm(pred_all, tgt_all)

        # ===== 新增：排除类别3和4的指标计算 =====
        # 创建排除类别3和4的目标标签副本
        ignore_idx = self.num_classes  # 使用num_classes作为ignore索引
        tgt_excl = tgt_all.clone()
        
        # 将Ground Truth中的类别3(Vehicle)和4(Pedestrian)标记为ignore
        mask_vehicle_pedestrian = (tgt_excl == 3) | (tgt_excl == 4)
        if mask_vehicle_pedestrian.any():
            tgt_excl[mask_vehicle_pedestrian] = ignore_idx
        
        # 更新排除类别3和4的混淆矩阵
        self.test_cm_excl(pred_all, tgt_excl)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        if self.trainer.datamodule.hparams.submit:
            self.trainer.datamodule.test_dataset.finalize_submission(self.submission_dir)

        # If test set misses target data, reset metrics and skip logging
        if not self.test_has_target:
            self.test_cm.reset()
            if hasattr(self, 'test_cm_excl'):
                self.test_cm_excl.reset()
            return
        ...
        if isinstance(self.logger, WandbLogger):
            self.logger.experiment.log({
                "test/cm": wandb_confusion_matrix(epoch_cm.confmat, class_names=self.class_names)
            })

        # ---- 新增：排除类别3和4的指标聚合和日志 ----
        if hasattr(self, 'test_cm_excl'):
            if self.trainer.num_devices > 1:
                epoch_cm_excl = torch.sum(self.all_gather(self.test_cm_excl.confmat), dim=0)
                epoch_cm_excl = ConfusionMatrix(self.num_classes).from_confusion_matrix(epoch_cm_excl)
            else:
                epoch_cm_excl = self.test_cm_excl

            # 记录排除类别3和4后的主要指标
            miou_excl = epoch_cm_excl.miou()
            oa_excl = epoch_cm_excl.oa()
            macc_excl = epoch_cm_excl.macc()
            
            self.log("test_excl34/miou", miou_excl, prog_bar=True, rank_zero_only=True)
            self.log("test_excl34/oa", oa_excl, prog_bar=True, rank_zero_only=True)
            self.log("test_excl34/macc", macc_excl, prog_bar=True, rank_zero_only=True)

            # 记录每个类别的IoU（排除类别3和4）
            for iou, seen, name in zip(*epoch_cm_excl.iou(), self.class_names):
                if seen and name not in ['Vehicle', 'Pedestrian']:  # 跳过类别3和4
                    self.log(f"test_excl34/iou_{name}", iou, prog_bar=False, rank_zero_only=True)

            # 可选：记录排除类别的混淆矩阵到wandb
            if isinstance(self.logger, WandbLogger):
                # 创建不包含类别3和4的类名列表用于可视化
                class_names_excl = [name if i not in [3, 4] else f"{name}(excluded)" 
                                for i, name in enumerate(self.class_names)]
                self.logger.experiment.log({
                    "test_excl34/cm": wandb_confusion_matrix(
                        epoch_cm_excl.confmat, class_names=class_names_excl)
                })
