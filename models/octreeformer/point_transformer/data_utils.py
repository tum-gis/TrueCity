...
def default_transforms(train=False, config=None, use_global_norm=False, norm_stats=None):
    """Create transform pipeline - supports both old and new signatures"""
    # Handle old signature (use_global_norm, norm_stats)
    ...
    # Handle new signature (train, config)
    return transforms.Compose([
        Normalize(),
        (lambda pc: (
...
def dataloader_tumfacade(lxyz, config, shuffle=False, use_global_norm=False, norm_stats=None):
    """Create dataloader for original data"""
def dataloader_tumfacade(lxyz, config, is_train=False, shuffle=False):
...
def create_preprocessed_dataloaders(config):
    ...
    eval_transforms = default_transforms(train=False, config=config)
