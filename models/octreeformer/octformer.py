...
class CPE(torch.nn.Module):
  ...
  def __init__(self, in_channels: int, kernel_size: List[int] = [3],
    ...
    allow_dwconv = (os.getenv("OCTREEFORMER_USE_DWCONV", "1") != "0")
    has_dwconv = importlib_util.find_spec("dwconv") is not None
    use_dw = bool(use_dwconv and allow_dwconv and has_dwconv)

    if use_dw:
      import dwconv  # only import when explicitly allowed and available
...
class OctFormer(torch.nn.Module):
  ...
  def __init__(self, in_channels: int,
               ...
               nempty: bool = True, stem_down: int = 2, use_dwconv: bool = False,
               **kwargs):
    super().__init__()
    self.patch_size = patch_size
    self.dilation = dilation
