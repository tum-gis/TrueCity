...
def main(cfg=None, accum_steps=1):
    if cfg is None:
        cfg = get_config()
    ...
    with tqdm(total=total_steps, desc="Training Progress", unit="batch") as pbar:
        for epoch in range(cfg.epochs):
            ...
            for i, (points, feat, target, offset) in enumerate(train_loader):
                ...
                union_meter.update(union)
                target_meter.update(target)
                # Scale back for logging (since we divided by accum_steps)
                loss_meter.update(loss.item() * accum_steps, n)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Point Transformer v1 Training")
    parser.add_argument(
        "--base_data_root",
        type=str,
        default=None,
        help="Base path to datav2_final (overrides Config.base_data_root)",
    )
    parser.add_argument(
        "--real_ratio",
        type=int,
        default=None,
        help="Real ratio (0, 25, 50, 75, 100) to select datav2_<ratio>_octree_fps",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        default=None,
        help="Batch size (overrides Config.batch_size, default: 32)",
    )
    parser.add_argument(
        "--accum_steps",
        type=int,
    ...
    cfg_kwargs = {}
    if args.base_data_root is not None:
        cfg_kwargs["base_data_root"] = args.base_data_root
    if args.real_ratio is not None:
        cfg_kwargs["real_ratio"] = args.real_ratio
    ...
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        print(f"[INFO] CUDA cache cleared. Free memory: {torch.cuda.get_device_properties(0).total_memory / 1024**3:.2f} GB")
    
    main(cfg, accum_steps=args.accum_steps)
